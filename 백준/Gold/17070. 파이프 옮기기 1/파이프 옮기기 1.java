...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		moveCount = 0;
		dfs(1, 2, 0);	// [1][2] 좌표 지점에서 시작, 방향은 가로방향(0)
	...
	public static void dfs(int nowX, int nowY, int direction) {
		...
		// 파이프 방향이 가로 방향(동쪽)으로 놓여진 경우
		// 가로 방향으로 놓여진 경우에는 가로(동쪽), 오른쪽 대각선 방향으로 놓을 수 있다.
		if(direction == 0) {
		...
		// 세로 방향으로 놓여진 경우에는 세로(아래), 오른쪽 대각선 방향으로 놓을 수 있다.
		else if(direction == 1) {
			// 세로 방향으로 이동 가능하면서 동시에 이동했을 때 해당 좌표가 빈칸(0)인 경우 (즉, 아래 방향으로 파이프 놓을 수 있는 경우)ㄴ
			if(nowX+1 <= N && map[nowX+1][nowY] == 0) {
				dfs(nowX+1, nowY, 1);	// 파이프 세로로 놓기 (direction = 1)
			}
		}
		// 파이프 방향이 오른쪽 대각선 방향으로 놓여진 경우
		// 오른쪽 대각선 방향으로 놓여진 경우에는 가로(동쪽), 세로(아래), 오른쪽 대각선 방향으로 놓을 수 있다.
		else if(direction == 2) {
			// 가로(동쪽) 방향으로 이동 가능하면서 동시에 이동했을 때 해당 좌표가 빈칸(0)인 경우 (즉, 동쪽 방향으로 파이프 놓을 수 있는 경우)
			if(nowY+1 <= N && map[nowX][nowY+1] == 0) {
				dfs(nowX, nowY+1, 0);	// 파이프 가로로 놓기 (direction = 0)
			}
			// 세로(아래) 방향으로 이동 가능하면서 동시에 이동했을 때 해당 좌표가 빈칸(0)인 경우 (즉, 아래 방향으로 파이프 놓을 수 있는 경우)
			if(nowX+1 <= N && map[nowX+1][nowY] == 0) {
				dfs(nowX+1, nowY, 1);	// 파이프 세로로 놓기 (direction = 1)
			}
		...
		// 대각선 방향으로 진행할 때 세 곳은 빈칸이여야 한다. (문제에서 색칠 된 곳 => 오른쪽, 오른쪽 대각선, 아래)
